...
public class LabeledScoredTreeNodeIterator {
	...
	private void visitTree(LabeledScoredTreeNode parent,
			LabeledScoredTreeNode tree, ILabeledScoredTreeNodeVisitor visitor)
			...
			if (words.hasNext()) {
				...
				if (words.hasNext()) {
					throw new TextParserException(
							"parent yielded more than one label");
				}
			} else {
	...
	public void visit(ILabeledScoredTreeNodeVisitor visitor)
			throws TextParserException {
		visitTree(null, _tree, visitor);
		visitor.visitsFinished();
	}
}
